...
def address_p2sh(
    redeem_script_hash: bytes, coin: CoinInfo, blinding_pubkey=None
) -> str:
...
def address_p2wpkh_in_p2sh(
    ...
    if derive_blinding_pubkey:
        blinding_pubkey = derive_blinding_pubkey(redeem_script)
    return address_p2sh(redeem_script_hash, coin, blinding_pubkey)
