...
class Confidential:
    ...
    def __init__(
        ...
        self.nonce = nonce  # for rangeproof generation on host
...
async def sign_tx(tx: SignTx, keychain: seed.Keychain, context: secp256k1_zkp.Context):
    ...
    if coin.confidential_assets:
        ...
        for o in range(tx.outputs_count):
            # The client will create the proofs (since it already knows the amounts and assets).
            writers.write_varint(tx_ser.serialized_tx, 0)  # surjection proof
            nonce = confidential_outputs[o].nonce
            if nonce is None:
                nonce = b""
            writers.write_varint(
                tx_ser.serialized_tx, len(nonce)
            )  # HACK: ECDH nonce instead of output amount rangeproof
            writers.write_bytes(tx_ser.serialized_tx, nonce)
    else:
        writers.write_uint32(tx_ser.serialized_tx, tx.lock_time)
