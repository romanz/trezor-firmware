...
class TestCryptoSecp256k1Zkp(Secp256k1Common, unittest.TestCase):
    ...
    def test_ecdh_compressed(self):
        for _ in range(100):
            sk = self.impl.generate_secret()
            pk = self.impl.publickey(sk)
            uncompressed = self.impl.multiply(sk, pk, False)
            compressed = self.impl.multiply(sk, pk, True)
            self.assertEqual(self.impl.multiply(sk, pk), uncompressed)
            self.assertEqual(compress_pubkey(uncompressed), compressed)
    ...
    def test_blind(self):
        ...
        for n_bits in (1, 2, 4, 8, 16, 24, 32, 40, 48, 56, 63):
            for _ in range(10):
                ...
                assert proof_buffer[:len(range_proof_view)] == range_proof_view

                asset_message_buf = proof_buffer  # reuse proof buffer
                assert len(asset_message_buf) >= 4096
                result = self.impl.rangeproof_rewind(
                    conf_value, conf_asset, nonce, range_proof_view, extra_commit,
                    asset_message_buf)
                assert result == (value, value_blind)
                assert asset_message_buf[:len(asset_message)] == asset_message

    def test_surjection_distinct_assets(self):
        proof_buffer = secp256k1_zkp.allocate_proof_buffer()
    ...
    def test_surjection_same_asset(self):
        proof_buffer = secp256k1_zkp.allocate_proof_buffer()
        ...
        for _ in range(100):
            ...
            output_asset = random.bytes(32)
            output_asset_blind = random.bytes(32)
            # All input assets are the same as the output asset:
            input_assets = output_asset * input_assets_len
            input_assets_blinds = random.bytes(32 * input_assets_len)
