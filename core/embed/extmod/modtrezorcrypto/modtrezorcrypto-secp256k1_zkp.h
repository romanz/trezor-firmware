...
STATIC mp_obj_t mod_trezorcrypto_secp256k1_context_rangeproof_sign(
  ...
  if (!secp256k1_rangeproof_sign(
          ...
          value, message.buf, message.len, extra_commit.buf,
          extra_commit.len, &generator)) {
    mp_raise_ValueError("Rangeproof sign failed");
  }
  if (rangeproof_len > RANGEPROOF_SIGN_BUFFER_SIZE) {
    mp_raise_ValueError("Too large rangeproof");
  }
